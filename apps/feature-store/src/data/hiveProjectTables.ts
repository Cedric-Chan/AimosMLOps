...
export const HIVE_PROJECT_TABLES: Record<string, string[]> = {
  ...
  risk_realtime: ["project_access_demo", "allowed_events_v1"],
...
export function getHiveProjectSchemaNames(): string[] {
  return Object.keys(HIVE_PROJECT_TABLES).sort((a, b) => a.localeCompare(b));
}

export function getHiveProjectTablesForSchema(schema: string): string[] {
  const list = HIVE_PROJECT_TABLES[schema.trim()];
